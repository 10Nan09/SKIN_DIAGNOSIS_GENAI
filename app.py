...
if GEMINI_API_KEY:
    ...
    print("⚠️ Warning: GEMINI_API_KEY not set. AI recommendations will be disabled.")
...
def generate_response(user_input):
    ...
    if not GEMINI_AVAILABLE:
        return "AI recommendations are currently unavailable. Please configure GEMINI_API_KEY environment variable."
...
def get_remedy_suggestions(pred_label):
    """Get remedy and medicine suggestions using Gemini"""
    if not GEMINI_AVAILABLE:
        return "AI recommendations are currently unavailable. Please configure GEMINI_API_KEY environment variable."
    ...
    try:
        ...
        return response.text.strip() if response.text else "Unable to generate suggestions at this time."
    except Exception as e:
        print(f"Gemini API Error: {e}")
...
def live_predict():
    ...
    image_path = None
    try:
        image_path = os.path.join(app.config['UPLOAD_FOLDER'], f"{session['user_id']}_live_{datetime.now().timestamp()}_{image_file.filename}")
        image_file.save(image_path)
        
        # Use image inference (live prediction mode)
        pred_label, probs = inference_image(image_path)
        suggestions = get_remedy_suggestions(pred_label)
